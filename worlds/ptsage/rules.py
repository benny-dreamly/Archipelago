...
def set_all_rules(world: SAGEWorld) -> None:
...
#Covered in regions, i think maybe
#def set_all_entrance_rules(world: SAGEWorld) -> None:
    # First, we need to actually grab our entrances. Luckily, there is a helper method for this.
    #hub_to_pizzascape = world.get_entrance("Hub to Pizzascape")
    #hub_to_ancient_cheese = world.get_entrance("Hub to Ancient Cheese")
    #right_room_to_final_boss_room = world.get_entrance("Hub to Bloodsauce Dungeon")
...
def set_all_location_rules(world: SAGEWorld) -> None:

    snick_challenge = world.get_location("Snick's Challenge Complete")
    cheese_sausage = world.get_location("Ancient Cheese Toppin (Sausage)")
    cheese_secret3 = world.get_location("Ancient Cheese Secret 3")
    cheese_secret4 = world.get_location("Ancient Cheese Secret 4")
    cheese_secret5 = world.get_location("Ancient Cheese Secret 5")
    cheese_secret6 = world.get_location("Ancient Cheese Secret 6")
    ...
    set_rule(cheese_secret4, lambda state: state.has("Bomb Transformation", world.player))
    set_rule(cheese_secret5, lambda state: state.has("Bomb Transformation", world.player))
    set_rule(cheese_secret6, lambda state: state.has("Bomb Transformation", world.player))
    set_rule(cheese_complete, lambda state: state.has("Bomb Transformation", world.player))

    if world.options.SRanks:
        cheese_srank = world.get_location("Ancient Cheese S Rank")
        set_rule(cheese_srank, lambda state: state.has("Bomb Transformation", world.player))
...
def set_completion_condition(world: SAGEWorld) -> None:
    # Finally, we need to set a completion condition for our world, defining what the player needs to win the game.
    # You can just set a completion condition directly like any other condition, referencing items the player receives:
    world.multiworld.completion_condition[world.player] = lambda state: state.has("Snick's Challenge Complete", world.player)
