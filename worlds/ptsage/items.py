...
ITEM_NAME_TO_ID = {
    "Pizzascape Access": 1,
    "Ancient Cheese Access": 2,
    "Bloodsauce Dungeon Access": 3,
    "Snick's Challenge Access": 4,
    "Knight Transformation": 5,
    "Bomb Transformation": 6,
    "Firebutt Transformation": 7,
    "Treasure (Pizzascape)": 8,
    "Treasure (Ancient Cheese)": 9,
    "Treasure (Bloodsauce Dungeon)": 10,
    "Toppin": 11,
    "Character Swapper": 12,
    "Shotgun": 13,
    "Pizza Shield": 14,
    "Snick.exe Trap": 15,
    "Surprise Bomb Trap": 16,
    "100 Points": 17,
}
...
DEFAULT_ITEM_CLASSIFICATIONS = {
    "Pizzascape Access": ItemClassification.progression,
    "Ancient Cheese Access": ItemClassification.progression,
    "Bloodsauce Dungeon Access": ItemClassification.progression,
    "Snick's Challenge Access": ItemClassification.progression,
    "Knight Transformation": ItemClassification.useful,
    "Bomb Transformation": ItemClassification.progression,
    "Firebutt Transformation": ItemClassification.useful,
    "Treasure (Pizzascape)": ItemClassification.filler,
    "Treasure (Ancient Cheese)": ItemClassification.filler,
    "Treasure (Bloodsauce Dungeon)": ItemClassification.filler,
    "Toppin": ItemClassification.progression_skip_balancing,
    "Character Swapper": ItemClassification.useful,
    "Shotgun": ItemClassification.useful,
    "Pizza Shield": ItemClassification.useful,
    "Snick.exe Trap": ItemClassification.trap,
    "Surprise Bomb Trap": ItemClassification.trap,
...
def get_random_filler_item_name(world: SAGEWorld) -> str:
    ...
    if world.random.randint(0, 99) < world.options.TrapChance:
        if world.random.randint(0, 1) == 0:
            return "Snick.exe Trap"
        else:
            return "Surprise Bomb Trap"
    return "100 Points"


def create_item_with_correct_classification(world: SAGEWorld, name: str) -> SAGEItem:
    ...
    if name == "Treasure (Pizzascape)" and world.options.Treasures:
        classification = ItemClassification.progression

    if name == "Treasure (Ancient Cheese)" and world.options.Treasures:
        classification = ItemClassification.progression

    if name == "Treasure (Bloodsauce Dungeon)" and world.options.Treasures:
        classification = ItemClassification.progression

    return SAGEItem(name, classification, ITEM_NAME_TO_ID[name], world.player)


# With those two helper functions defined, let's now get to actually creating and submitting our itempool.
def create_all_items(world: SAGEWorld) -> None:
    ...
    itempool: list[Item] = [
        world.create_item("Pizzascape Access"),
        world.create_item("Ancient Cheese Access"),
        world.create_item("Bloodsauce Dungeon Access"),
        world.create_item("Snick's Challenge Access"),
        world.create_item("Knight Transformation"),
        world.create_item("Bomb Transformation"),
        world.create_item("Firebutt Transformation")

    ]
    itempool += [world.create_item("Toppin") for _ in range(toppinpool)]
