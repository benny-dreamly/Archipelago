...
from .Items import Smash64Item, item_data_table, item_table, item_filler, item_filler_weight
from .Locations import Smash64Location, location_data_table, location_table, locked_locations
from .Options import Smash64Options
from .Regions import region_data_table
from .Rules import *
from .Rom import MD5Hash, Smash64ProcedurePatch, write_tokens
...
class Smash64Settings(settings.Group):
    class RomFile(settings.UserFilePath):
        """File name of the Smash 64 US rom"""
        copy_to = "Smash64.z64"
        description = "Smash 64 (US) ROM File"
        md5s = MD5Hash
...
class Smash64WebWorld(WebWorld):
    ...
    setup_en = Tutorial(
        ...
        description="A guide to playing Smash 64.",
        language="English",
...
class Smash64World(World):
    ...
    def create_item(self, name: str) -> Smash64Item:
        return Smash64Item(name, item_data_table[name].type, item_data_table[name].code, self.player)
    
    def create_items(self) -> None:
        item_pool: List[Smash64Item] = []
    ...
    def generate_output(self, output_directory: str):
        ...
        self.rom_name_text = f'S64{Utils.__version__.replace(".", "")[0:3]}_{self.player}_{self.multiworld.seed:11}\0'
        self.romName = bytearray(self.rom_name_text, "utf8")[:0x20]
        self.romName.extend([0] * (0x20 - len(self.romName)))
        self.rom_name = self.romName
        self.playerName = bytearray(self.multiworld.player_name[self.player], "utf8")[:0x20]
        self.playerName.extend([0] * (0x20 - len(self.playerName)))
        patch = Smash64ProcedurePatch(player=self.player, player_name=self.multiworld.player_name[self.player])
